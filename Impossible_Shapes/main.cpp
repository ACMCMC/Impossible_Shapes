...
void openGlInit() {
	...
	glClearColor(1.0f, 1.0f, 1.0f, 1.0f); //Borro el Buffer the la ventana
...
int main()
{
	...
	// Creo la ventana
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Proyecto Aldan Creo Marino", NULL, NULL);
	if (window == NULL)
	{
	...
	GLuint cemento = cargarTextura("bola.jpg");

	penroseFrontal = Objeto(glm::vec3(0.0f, 1.0f, 1.0f), VAOPenroseFrontal, 99, shaderSinTexturas);
	penroseFrontal.setDistorsion(glm::vec3(0.05, 0.05, 0.05));

	penroseTrasero = Objeto(glm::vec3(0.0f, 1.0f, 1.0f), VAOPenroseTrasero, 99, shaderSinTexturas);
	penroseTrasero.setDistorsion(glm::vec3(0.05, 0.05, 0.05));

	penroseLateral = Objeto(glm::vec3(0.0f, 1.0f, 1.0f), VAOPenroseLateral, 99, shaderSinTexturas);
